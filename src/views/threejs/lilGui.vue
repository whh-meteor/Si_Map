...
export default {
  ...
  methods: {
    init() {
      ...
      let folder = gui.addFolder("显示控制");
      folder.add(myObject, 'fullScreenBtnFunction').name('全屏显示') // Checkbox
      folder.add(myObject, 'exitFullScreenBtnFunction').name('退出全屏') // Button

      let folder2 = gui.addFolder("位置控制");
      folder2.add(this.mesh.position, "x").min(-5).max(5).step(1).name("设置立方体x轴距离").onChange((value) => {
        console.log("数值被改变为", value);
      });
      folder2.add(this.mesh.position, "y").min(-5).max(5).step(1).name("设置立方体y轴距离");
      folder2.add(this.mesh.position, "z").min(-5).max(5).step(1).name("设置立方体z轴距离");
      let folder3 = gui.addFolder("样式控制");
      folder3.add(material, "wireframe").name("控制元素为线框模式");

      let colorParams = {
        meshColor: 0x08ffe
      }
      folder3.addColor(colorParams, "meshColor").name("颜色设置").onChange((value) => {
